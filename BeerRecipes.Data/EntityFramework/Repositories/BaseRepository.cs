...
namespace BeerRecipes.Data.EntityFramework.Repositories
{
        ...
        where TEntity : class, IEntity<TKey>, new()
    {
        ...
        public Task<List<TEntity>> GetAllContainsName(string name)
        {
            _logger.LogInformation("Get {type} with name = {name}", typeof(TEntity).Name, name);
            return _db.Set<TEntity>().Where(c => c.Name.Contains(name)).ToListAsync();
        }
        ...
        public virtual Task<TEntity> Get(string name)
        {
            _logger.LogInformation("Get {type} with name = {name}", typeof(TEntity).Name, name);
            return _db.Set<TEntity>().SingleOrDefaultAsync(c => c.Name.Equals(name));
        }
